...
@Component({
  ...
  template: `
    <div *ngIf="datepickerMode" role="application" ><!--&lt;!&ndash;ng-keydown="keydown($event)"&ndash;&gt;-->
      <ng-content></ng-content>
    </div>
  `
})

    /*class="well well-sm bg-faded p-a card" */
export class DatePickerInnerComponent implements OnInit, OnChanges {
  ...
  @Output() public monthChange: EventEmitter<Date> = new EventEmitter<Date>(false);
  ...
  public triggerMonthChange(change: number) {
      let nextDate : Date = new Date(this.activeDate.getFullYear(), this.activeDate.getMonth() + change, 1);
      this.monthChange.emit(nextDate);
  }
